...
public:
    ...
    void block_conv(const int * images, const int * filters, int * result, 
        ...
        for (b = 0;b < block_config.block_B; ++b)
        {
            for (c = 0;c < block_config.block_C; ++c)
            {
                for (k = 0; k < block_config.block_K; ++k)
                {
                    for (w = 0; w < block_config.block_W; ++w)
                    {
                        for (h = 0; h < block_config.block_H; ++h)
                        {
                            for (rp = 0 ; rp < block_config.block_Rp; ++rp)
                            {
                                for (rpp = 0 ; rpp < block_config.block_Rpp; ++rpp)
                                {
                                    for (sp = 0; sp < block_config.block_Sp; ++sp)
                                    {
                                        for (spp = 0; spp < block_config.block_Spp; ++spp)
                                        {
